...
export default function DottedBg2({
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        isPlayingRef.current = effectivePlay && entry.isIntersecting;
        if (isPlayingRef.current && !rafIdRef.current) {
          lastTimeRef.current = performance.now();
          rafIdRef.current = requestAnimationFrame(update);
        } else if (!isPlayingRef.current && rafIdRef.current) {
  ...
  return (
    ...
    >
      ...
      />
      <div ref={containerRef} style={{ position: "absolute", inset: 0, opacity: 0.3 }} />
    </div>
  );
}
